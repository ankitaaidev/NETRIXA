...
import siteConfiguration from './.make/make/site.json'

// Vite config — https://vitejs.dev/config/
export default defineConfig(({ mode }) => {
  // The deploy-preview command passes `--mode development` for cached-preview builds.
...
/** Applies /.make/make/site.json to the generated document shell. */
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? 'NETRIXA'
...
/**
 * Serves a blank render-target page at /.make/make/kit.html that
...
function figmaMakeKitPlugin(options: { storiesGlob: string | string[] }): Plugin {
  ...
  const ROUTE = '/.make/make/kit.html'
