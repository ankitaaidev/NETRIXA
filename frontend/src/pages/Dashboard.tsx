...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="glass px-3 py-2 text-xs">
      <div className="font-mono-data text-[10px] text-[#7a9cc4] mb-1">
        {label}
      </div>
      {payload.map((p: any) => (
  ...
  const KPI = [
    {
      ...
      sub: "FIRMS detections processed",
      icon: Flame,
      accent: "cyan",
    },
    {
      label: "Industrial Context",
      value: summary.industrialEvents.toLocaleString(),
      sub: "Near industrial facilities",
      icon: Building2,
      accent: "cyan",
    },
    {
      label: "Potential Ind. Fires",
      value: summary.potentialIndustrialFires.toString(),
      sub: "AI-flagged anomalies",
      icon: AlertTriangle,
      accent: "orange",
    },
    {
    ...
    },
    {
      ...
      value: summary.persistentSources.toString(),
      sub: "Recurring thermal activity",
      icon: Activity,
      accent: "amber",
    },
  ];
  ...
  const trend = useMemo(() => {
    const days = Array.from({ length: 14 }, (_, index) => {
      ...
      return {
        ...
        industrial: dayEvents.filter(
          (event) =>
            event.industrialProximity === "HIGH" ||
            event.industrialProximity === "MEDIUM",
        ).length,
        critical: dayEvents.filter((event) => event.riskLevel === "CRITICAL")
          .length,
      };
    });
    return days;
  ...
  return (
    <div className="flex flex-col h-full bg-[#050a14]">
      ...
      <div className="px-5 py-4 border-b border-[#1e3a5f] flex items-start justify-between flex-shrink-0">
        <div>
          <div className="font-display font-700 text-2xl tracking-[0.12em] text-[#e2eaf5]">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono-data text-[10px] text-green-400 tracking-widest">
            FIRMS LIVE · AUTO SYNC
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-0 border-b border-[#1e3a5f] flex-shrink-0">
        {KPI.map((k) => (
          <div
            key={k.label}
            className={`kpi-card ${k.accent} border-r border-[#1e3a5f] last:border-r-0`}
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-mono-data text-[10px] text-[#3d6490] tracking-widest uppercase mb-1">
                  {k.label}
                </div>
                <div className="font-display font-700 text-3xl text-[#e2eaf5] leading-none">
                  {k.value}
                </div>
                <div className="font-mono-data text-[9px] text-[#3d6490] mt-1 tracking-wide">
                  {k.sub}
                </div>
              </div>
              <k.icon
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 relative min-w-0">
          ...
          {(loading || eventsError) && (
            <div className="absolute top-3 left-3 glass px-3 py-1.5 font-mono-data text-[10px] text-amber-400">
              {loading ? "SYNCING MAP DATA..." : "DEMO MAP DATA ACTIVE"}
            </div>
          )}
            ...
            <div className="absolute top-3 right-3 w-64 glass z-10">
              ...
              <div className="p-3">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="font-mono-data text-[10px] text-[#7a9cc4] tracking-widest">
                      {selectedEvent.eventId}
                    </div>
                    <div className="font-display font-600 text-sm text-[#e2eaf5] mt-0.5 leading-tight">
                      {selectedEvent.classification}
                    </div>
                  </div>
                  <button
                    onClick={() => setSelectedEvent(null)}
                    className="text-[#3d6490] hover:text-[#7a9cc4] text-xs leading-none ml-2 mt-0.5"
                  >
                    ✕
                  </button>
                </div>
                ...
                <div className="mt-3 space-y-1.5 border-t border-[#122035] pt-2">
                  {[
                    ...
                    selectedEvent.facilityName
                      ? [
                          "Nearest Facility",
                          `${selectedEvent.facilityDistance} m`,
                        ]
                      : ["Land Cover", selectedEvent.landCover],
                    [
                      "Historical Deviation",
                      `${selectedEvent.zScore > 2 ? "HIGH" : selectedEvent.zScore > 1 ? "MEDIUM" : "LOW"} (z=${selectedEvent.zScore}σ)`,
                    ],
                    ["Observations", `${selectedEvent.observations}`],
                  ].map(([k, v]) => (
                    <div key={k} className="flex justify-between items-center">
                      <span className="font-mono-data text-[9px] text-[#3d6490] tracking-wide">
                        {k}
                      </span>
        ...
        <div className="w-72 flex flex-col border-l border-[#1e3a5f] flex-shrink-0 overflow-y-auto">
          ...
          <div className="p-4 border-b border-[#1e3a5f]">
            <div className="font-mono-data text-[10px] text-[#3d6490] tracking-widest mb-3">
              RECENT EVENT TREND
            </div>
              ...
              {events
                ...
                .map((evt) => (
                  ...
                  >
                    ...
                    <div className="font-display text-xs text-[#7a9cc4] leading-tight">
                      {evt.classification}
                    </div>
