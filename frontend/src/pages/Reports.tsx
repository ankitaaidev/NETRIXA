...
export default function Reports() {
  ...
  const [events, setEvents] = useState<ThermalEvent[]>(
    ALL_EVENTS.map((event) => ({ ...event, id: event.eventId })),
  );
  ...
  const [report, setReport] = useState<Awaited<
    ReturnType<typeof api.generateReport>
  > | null>(null);
    ...
    <div className="flex flex-col h-full">
      <div className="px-5 py-4 border-b border-[#1e3a5f] flex-shrink-0">
        ...
        <div className="font-mono-data text-[10px] text-[#3d6490] tracking-widest">
          GENERATE STRUCTURED INTELLIGENCE REPORTS FOR THERMAL EVENTS
        </div>
      </div>
      {error && (
      ...
      <div className="flex-1 overflow-auto p-5">
        <div className="max-w-3xl space-y-4">
              ...
              >
                ...
                {events.map((e) => (
                  <option key={e.eventId} value={e.eventId}>
                    {e.eventId} · {e.classification} · {e.riskLevel}
                  </option>
                ))}
              </select>
              <button
          ...
          {/* Report preview */}
          {loading && (
            <div className="font-mono-data text-[10px] text-cyan-400">
          ...
          {generated && event && report && (
            <div className="bg-[#0a1628] border border-[#1e3a5f]">
              {/* Report header */}
              <div
                className="px-6 py-4 border-b border-[#1e3a5f] flex items-start justify-between"
                style={{
                  borderTopColor:
              ...
              >
                <div>
                  <div className="font-mono-data text-[9px] text-[#3d6490] tracking-widest mb-1">
                    NETRIXA INTELLIGENCE REPORT
                  </div>
                  <div className="font-display font-700 text-lg text-[#e2eaf5]">
                    {event.eventId}
                  </div>
                  <div className="font-display text-sm text-[#7a9cc4] mt-0.5">
                ...
                <button
                  onClick={async () => {
                    try {
                      const blob = await api.downloadReportPdf(selectedId);

                      const url = window.URL.createObjectURL(blob);
                      const link = document.createElement("a");

                      link.href = url;
                      link.download = `netrixa-report-${selectedId}.pdf`;
                  ...
                  className="flex items-center gap-1.5 bg-[#050a14] border border-[#1e3a5f] text-[#7a9cc4] font-mono-data text-[10px] px-3 py-2 hover:text-[#e2eaf5] transition-colors"
                >
                  <Download size={12} />
                  DOWNLOAD PDF
                </button>
              ...
              <div className="p-6 space-y-5 font-mono-data text-[11px]">
                {/* Event info */}
                <section>
                  <div className="text-[9px] text-[#3d6490] tracking-widest mb-2 border-b border-[#0d1f3c] pb-1">
                  ...
                  <div className="grid grid-cols-2 gap-x-8 gap-y-1.5">
                    {[
                      ...
                      <div key={k} className="flex gap-2">
                        <span className="text-[#3d6490] w-40 flex-shrink-0">
                          {k}:
                        </span>
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 gap-x-8 gap-y-1.5">
                    {[
                      ...
                      [
                        "Anomaly Score",
                        `${(event.anomalyScore * 100).toFixed(0)} / 100`,
                      ],
                    ].map(([k, v]) => (
                      <div key={k} className="flex gap-2">
                        <span className="text-[#3d6490] w-40 flex-shrink-0">
                    ...
                    {event.explanation
                      ...
                      .map((f) => (
                        <div key={f.factor} className="flex items-start gap-2">
                          ...
                          />
                          <span className="text-[#7a9cc4]">
                            {f.factor}: {f.description}
                          </span>
                        </div>
                      ))}
                  </div>
                  <p className="mt-2 text-[9px] text-[#3d6490]">
                    * {report.disclaimer}
                  </p>
                </section>
                ...
                <section>
                  ...
                  <div
                    className="bg-[#050a14] border px-3 py-2"
                    style={{
                      borderColor:
                        event.riskLevel === "CRITICAL"
                          ? "#ef444440"
